...
( function () {
    ...
    /* ── State ── */
    let discoverySeed   = Math.floor( Math.random() * 1000000 );
    let seenVideos      = [];
    ...
    async function loadNextPage() {
        ...
        if ( ! hasMore ) {
            ...
            currentPage = 1;
            hasMore = true;
            discoverySeed = Math.floor( Math.random() * 1000000 ); // Fresh shuffle for the loop
        }
        ...
        try {
            ...
            url.searchParams.set( 'space', currentSpace );
            url.searchParams.set( 'seed', discoverySeed );
            url.searchParams.set( 'order_by', 'discovery' );
            
            if ( seenVideos.length > 0 ) {
                url.searchParams.set( 'exclude', seenVideos.join(',') );
            }
    ...
    function createSlide( video ) {
        ...
        videoEl.addEventListener( 'playing', () => {
            if ( activeSlide === slide ) {
                ...
                // If missing thumbnail, try to capture and save one (Self-Healing)
                if ( ! video.thumbnail_url && ! slide.dataset.thumbRequested ) {
                    slide.dataset.thumbRequested = 'true';
                    setTimeout( () => captureAndSaveThumbnail( videoEl, video.id ), 500 );
                }
            }
        }, { once: true } );
    ...
    async function activateSlide( slide ) {
        ...
        // Track as "Seen"
        const videoId = parseInt( slide.dataset.id );
        if ( videoId && ! seenVideos.includes( videoId ) ) {
            seenVideos.push( videoId );
            // Keep the last 200 seen videos
    ...
    /**
     * Capture a frame from a video and send it to the server to save as a thumbnail.
     */
    async function captureAndSaveThumbnail( videoEl, videoId ) {
        ...
        try {
            ...
            console.log( `FCM Reels: Thumbnail auto-generated and saved for video #${videoId}` );
        } catch ( e ) {
            // Silently fail
        }
    }
